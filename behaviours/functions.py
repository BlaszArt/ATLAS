...
class AlgorithmFunctions:
    ...
    def set_what_to_do_with_lights(agent):
        ...
        agent.directions_max_cars = agent.get_roads_with_max_cars()

        change_time_by = AlgorithmFunctions.cars_to_time(
            abs(agent.directions_max_cars['vertical'] - agent.directions_max_cars['horizontal']))
    ...
    def make_participant_proposal(participant, cfp_message):
        ...
        part_potential_change_time = AlgorithmFunctions.cars_to_time(
            abs(participant.directions_max_cars['vertical'] - participant.directions_max_cars['horizontal']))
        ...
        if (init_change_direction != part_potential_change_dir and
                init_change_for + participant.directions_max_cars[
                    part_potential_change_dir] < part_potential_change_time):
